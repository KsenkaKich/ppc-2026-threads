...
namespace vinyaikina_e_multidimensional_integrals_simpson_method {
  namespace {
    ...
    double count(double left_border,
      double right_border, double simpson_factor, std::vector<std::pair<double, double>>& limits, std::vector<double>& actual_step,
...
#pragma omp parallel num_threads(num_threads) default(none) shared(limits, simpson_factor, actual_step, delta, function, std::cout) reduction(+:res)
    {
